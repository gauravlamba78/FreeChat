...
import { fetch, update, deleteItem, setRefetch, deleteItemsByIds } from '../../stores/organizations/organizationsSlice'
...
const TableSampleOrganizations = ({ filterItems, setFilterItems, filters, showGrid }) => {
  ...
  useEffect(() => {
      if (organizationsNotify.showNotification) {
          notify(organizationsNotify.typeNotification, organizationsNotify.textNotification);
      }
  ...
  useEffect(() => {
    loadColumns(handleDeleteModalAction, `organizations`).then((newCols) =>
      setColumns(newCols),
    );
  }, []);
