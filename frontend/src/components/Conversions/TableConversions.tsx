...
import { fetch, update, deleteItem, setRefetch, deleteItemsByIds } from '../../stores/conversions/conversionsSlice'
...
const TableSampleConversions = ({ filterItems, setFilterItems, filters, showGrid }) => {
  ...
  const { conversions, loading, count, notify: conversionsNotify, refetch } = useAppSelector((state) => state.conversions)
  ...
  useEffect(() => {
      if (conversionsNotify.showNotification) {
          notify(conversionsNotify.typeNotification, conversionsNotify.textNotification);
      }
  ...
  useEffect(() => {
    loadColumns(handleDeleteModalAction, `conversions`).then((newCols) =>
      setColumns(newCols),
    );
  }, []);
